...
\title{Machine Translation Phrase Table Induction from Monolingual Data}
...
In the standard phrase-based statistical machine translation (SMT) pipeline, the table of phrase pairs used to translate, or the grammar, is derived from a word-aligned parallel corpus. Researchers have found that, in general, the larger the parallel corpus from which the grammar is derived, the better translation performance is. However, for nearly all of the world's languages, there is no corpus of text that has a parallel counterpart in any other language that is big enough to be used to derive a grammar and translate from the given language. Following the work of \cite{KlementievACL2011}, we consider the translation phrase table itself a parameter of SMT, and we explore using a seed dictionary and large monolingual corpora to derive it.

In this paper we:

\begin{itemize}
\item{Motivate the need for generating translation phrase tables from non-parallel sources}
\item{Describe the challenges involved, in particular the large computational cost of comparing all source language n-grams with all target language n-grams}
\item{Present several novel methods for pruning the phrase pair search space}
\item{Propose an intrinsic metric for measuring the quality of the phrase table}
\item{Report end-to-end translation performance for several language pairs}
\end{itemize}
...
\subsection{Monolingual estimation of phrase-based SMT parameters} \label{sect:bckg:smt}
...
%\caption{Collecting phrase orientation statistics for a en-de phrase pair (\emph{profile}, \emph{Profils}) from non-parallel sentences (the German sentence translates as  \emph{Creating a Facebook profile is easy}). The longest preceding phrase \emph{Facebook} in source, has a phrase table translation \emph{in Facebook} appearing after the target phrase.}
%\label{fig:monoreord}
%\end{center}
%\vskip -0.2in
%\end{figure}
...
\bibliography{lowresmt-phrasetable}
